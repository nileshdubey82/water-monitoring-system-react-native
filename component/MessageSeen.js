...
function ViewRequestpage({navigation}) {
 ...
 useEffect(() => {
  ...
  function ViewMessage(){
    ...
    var APIURL = "https://h2osavernil.000webhostapp.com/message-seen.php";
  ...
  const renderItem = ({ item }) => {
    return (
      <View onPress={()=>navigation.navigate('BlankPage', {
        item: item
      })} style={{flexDirection:"row",flex:1,borderStartColor:'lightgreen'}}>
        <View style={styles.txt}>
        <TouchableOpacity style={{left:20}} onPress={() => getItem(item)}>
          <Text > Receiver Name: {item.to_name}</Text>
...
const styles = StyleSheet.create({
  txt:{
     ...
     height:80,
     width:"100%",
    //  paddingHorizontal:23,    //  justifyContent:"center",
    //  flexDirection:'row'
    border:1
  },
  approve_btn:{
